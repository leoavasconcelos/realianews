...
const shareTargets = [
  ...
  },
  {
    ...
    color: 'bg-[#26A5E4] hover:bg-[#1E8DC4]',
    textColor: 'text-white',
    getUrl: (title: string, url: string) =>
...
const ShareSheet: React.FC<ShareSheetProps> = ({ open, onOpenChange, title, url }) => {
  const handleShare = (getUrl: (title: string, url: string) => string) => {
    window.open(getUrl(title, url), '_blank', 'noopener,noreferrer');
    onOpenChange(false);
  };
    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="pb-8">
        ...
        <div className="px-6 pb-2">
          <p className="text-sm text-muted-foreground line-clamp-2 text-center mb-6">{title}</p>
          ...
          <div className="grid grid-cols-5 gap-4 mb-6">
            {shareTargets.map((target) => (
              ...
              >
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center ${target.color} ${target.textColor} transition-transform active:scale-95`}
                >
                  <target.icon />
                </div>
